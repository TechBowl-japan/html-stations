        ...
        if (isLeft(res)) {
          ...
          cy.on('fail', (error, _) => {
            ...
            error.message =
              ...
              errors
                .map((err) => `(${err.type}) ${err.description ?? ''}`)
                .join('\n')
  ...
  it('Twitter用の有効な`<meta>`タグが存在する', function () {
    cy.get('meta[name="twitter:card"]')
      ...
      .then((cardMeta) => {
        ...
        expect(
          cardTypes.indexOf(cardType) !== -1,
          `Twitter card type should be one of : "${cardTypes.join('", "')}"`
        ).to.be.true
      })

    cy.get('meta[name="twitter:site"],meta[name="twitter:creator"]').then(
      (cardMeta) => {
        cardMeta.each((_, el) => {
          const twitterId = el.getAttribute('content')
          expect(twitterId).satisfy(
            (v) => /^@(\w){1,15}$/.test(v),
            'the values of twitter:site and twitter:creator should be valid Twitter ID'
          )
        })
      }
    )
